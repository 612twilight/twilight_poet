    ...
    def cangtou(self, given_text):
        ...
        for i in range(4):
            ...
            if i == 0:
                ...
                start += word  # 否则将之前生成的加上这个开头
            before_idx = len(start)  # 记录下之前的诗句开头
            if i != 3:  # 对于前三句话
                sample = self.model.sample_seq(self.sess, self.args.length, start,
                                               sample_type=SampleType.weighted_sample)
            else:
                if not word:
                    # 如果最后一个藏头字没有，则那么最后一句话的韵脚就得往后移动一位，因为要生成前一个字，否则就不用了
                    # 难道是第二句的末尾和
                    rhyme_seq += 1
            ...
            sample = sample[before_idx:]  # 生成的句子 这里生成的长度和 self.args.length 一样，大约是100
            idx1 = sample.find('，')
            idx2 = sample.find('。')
            min_idx = min(idx1, idx2)  # 找到最先生成的都好或者句号

            if min_idx == -1:  # 如果有一个没有，就用另一个
                if idx1 > -1:
                    min_idx = idx1
                else:
                    min_idx = idx2
            if min_idx > 0:
                ...
                # 加上后一句话，这句话是以逗号或者句号结尾的，这个标点要加上去
                if i == 1:  # 记录第二句的末尾的词作为押韵词
                    rhyme_seq = min_idx - 1

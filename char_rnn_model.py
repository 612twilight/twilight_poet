...
class CharRNNLM(object):
    def __init__(self, is_training, batch_size, num_unrollings, vocab_size, w2v_model,
        ...
        with tf.name_scope('embedding_layer'):
            ...
            # batch_size num_unrollings embedding_size
        ...
        with tf.name_scope('flatten_outputs'):
            # concat 之后是 batch hidden_zie*num_unrolling
            # reshape 之后是 batch_size*num_unrolling hidden_size
            flat_outputs = tf.reshape(tf.concat(axis=1, values=outputs), [-1, hidden_size])

        with tf.name_scope('flatten_targets'):
            # batch_size*num_unrolling
            flat_targets = tf.reshape(tf.concat(axis=1, values=self.targets), [-1])

        with tf.variable_scope('softmax') as sm_vs:
            softmax_w = tf.get_variable('softmax_w', [hidden_size, vocab_size])
            softmax_b = tf.get_variable('softmax_b', [vocab_size])
            self.logits = tf.matmul(flat_outputs, softmax_w) + softmax_b  # batch_size*num_unrolling vocab_size
        ...
        with tf.name_scope('loss_montor'):
            ...
            average_loss_summary = tf.summary.scalar(name='average loss', tensor=self.average_loss)
            ppl_summary = tf.summary.scalar(name='perplexity', tensor=self.ppl)

        self.summaries = tf.summary.merge(inputs=[average_loss_summary, ppl_summary],
                                          name='loss_monitor')
    ...
    def sample_seq(self, session, length, start_text, sample_type=SampleType.max_prob, given='', rhyme_ref='',
        ...
        for i in range(length):  # 根据之前生成的句子的最终向量，连续生成length长度的新句子
            ...
            if rhyme_ref and i == rhyme_idx:  # 这里是为了押韵，第二句的末尾和第四句的末尾要押韵
    ...
    def select_rhyme(self, rhyme_ref, probs):
        if rhyme_ref:
            ...
            if rhyme_set:  # 如果生成的第二句的韵脚在韵律字典里面，则取出全部的韵律字典
                seq_probs = zip(probs, range(0, self.vocab_size))
                topn = heapq.nlargest(50, seq_probs, key=lambda sp: sp[0])
                # 一句概率找到前50个，如果前50个里有，就选取前50里面最大的，否则选取最大概率的那个，就不管韵律了
                for _, seq in topn:

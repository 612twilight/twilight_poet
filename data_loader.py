...
class DataLoader():
    ...
    def create_batches(self, samples):
        ...
        sample_size = len(samples)  # 样本总数
        self.num_batches = math.ceil(sample_size / self.batch_size)  # 一轮的batch数目
        new_sample_size = self.num_batches * self.batch_size  # 规整后的样本数目，将增加了那些除不尽的不够的诗句
        ...
        for i in range(sample_size, new_sample_size):  # 补充一波，使其达到new_sample_size数目
            copyi = i - sample_size
            x_seqs[i] = x_seqs[copyi]
            x_lengths.append(x_lengths[copyi])

        y_seqs[:, :-1] = x_seqs[:, 1:]  # 生成模型的目标序列，移位  new_sample_size seq_max_length
        y_seqs[:, -1] = x_seqs[:, 0]  # 将生成的序列的最后一位以输入序列的第一位填充？
        x_len_array = np.array(x_lengths)

        # reshape之后就是 batch_size num_batch* seq_max_length
        self.x_batches = np.split(x_seqs.reshape(self.batch_size, -1), self.num_batches, 1)
        # 利用split函数，将其分割成num_batch个 batch_size seq_max_length 形状的数据
    ...
    def get_text_idx(text, vocab, max_document_length):
        ...
        for i, x in enumerate(text):
            ...
            if len(x) > max_document_length:
                x_parts = x[:max_document_length_without_end]  # 超过最大长度的，就截取到第一个句号的位置，
                # 然后补足成为[xxxx,xxxx。xxxx,xxxx。]最终诗句的应该有的样子是这样的，当然也可能出现没有句号，或者只剩下一个句号
                # 比如[xxxx,xxxx。]
                idx = x_parts.rfind('。')
                if idx > -1:
